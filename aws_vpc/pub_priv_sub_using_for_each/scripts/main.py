...
with Diagram("\nVPC with Public and Private Subnet in each AZ using FOR_EACH", show=False, graph_attr=dia_graph_attr) as diag:
    with Cluster(f"{' '*10}AWS Cloud", graph_attr=AWS_graph_attr):
        Node(label="", shape="plaintext", pin="true", pos="0,4" )
        Node(label="", shape="plaintext", pin="true", pos="10.6,8" )
...
# This determines the outline used for the Region 
    with Cluster(f"{' '*12}Region{' '*2}us-west-2", graph_attr=region_graph_attr):
        Node(label="", shape="plaintext", pin="true", pos="0.2,4.4" )
        Node(label="", shape="plaintext", pin="true", pos="10.2,7.8" )   
...
# This determines the outline used for the VPC
    with Cluster(f"{' '*12}VPC{' '*100}10.0.0.0/16", graph_attr=vpc_graph_attr):
        ...
        Node(label="", shape="plaintext", pin="true", pos="9.8,7.6" ) 
        # This is used for dispalying the VPC icon       
        vpc = VPC(label="", fontsize="6", loc="t",fixedsize="true", width="0.4", height="0.4", 
            pin="true", pos="-0.8,8.1")

# This determines the outline used for the ============Avalilabity Zone -1 ====================
    with Cluster(f"{' '*8}AZ1{' '*5}us-west-2a", graph_attr=az_graph_attr):
        Node(label="", shape="plaintext", pin="true", pos="0.1,5" )
        Node(label="", shape="plaintext", pin="true", pos="1,7" )        
        az1 = LocalZones(label="", fontsize="6", loc="t",fixedsize="true", width="0.3", height="0.3", 
            pin="true", pos="-0.5,7.8")

# This determines the outline used for the Public Subnet in AZ1
    with Cluster(f"{' '*8}Public Subnet\n\n10.0.0.0/24", graph_attr=public_graph_attr):
        Node(label="", shape="plaintext", pin="true", pos="0.3,6.8" )
        Node(label="", shape="plaintext", pin="true", pos="0.8,6.8" )  
        public_subnet_az1 = PublicSubnet(
            label="", fontsize="6", loc="t",fixedsize="true", width="0.3", height="0.3",
            pin="true", pos="-0.2,7.4", )

# This determines the outline used for the Private Subnet in AZ1
    with Cluster(f"{' '*8}Private Subnet\n\n10.0.4.0/24", graph_attr=private_graph_attr):
        Node(label="", shape="plaintext", pin="true", pos="0.3,5.2" )
        Node(label="", shape="plaintext", pin="true", pos="0.8,5.2" )  
        private_subnet_az1 = PrivateSubnet(
            label="", fontsize="6", loc="t",fixedsize="true", width="0.3", height="0.3",
            pin="true", pos="-0.2,5.8", )

# This determines the outline used for the ============Avalilabity Zone 2 ====================
    with Cluster(f"{' '*5}AZ2{' '*5}us-west-2b", graph_attr=az_graph_attr):
        Node(label="", shape="plaintext", pin="true", pos="2.8,5" )
        Node(label="", shape="plaintext", pin="true", pos="3.8,7" )        
        az2 = LocalZones(label="", fontsize="6", loc="t",fixedsize="true", width="0.3", height="0.3", 
            pin="true", pos="2.2,7.8")

# This determines the outline used for the Public Subnet in AZ2
    with Cluster(f"{' '*8}Public Subnet\n\n10.0.1.0/24", graph_attr=public_graph_attr):
        Node(label="", shape="plaintext", pin="true", pos="3,6.8" )
        Node(label="", shape="plaintext", pin="true", pos="3.6,6.8" )  
        public_subnet_az2 = PublicSubnet(label="", fontsize="6", loc="t",fixedsize="true", width="0.3", height="0.3",
            pin="true", pos="2.4,7.4", )

# This determines the outline used for the Private Subnet in AZ2
    with Cluster(f"{' '*8}Private Subnet\n\n10.0.5.0/24", graph_attr=private_graph_attr):
        Node(label="", shape="plaintext", pin="true", pos="3.6,5.2" )
        Node(label="", shape="plaintext", pin="true", pos="3.6,5.2" )  
        private_subnet_az2 = PrivateSubnet(label="", fontsize="6", loc="t",fixedsize="true", width="0.3", height="0.3",
            pin="true", pos="2.4,5.8", )

# This determines the outline used for the ============Avalilabity Zone 3 ====================
    with Cluster(f"{' '*5}AZ3{' '*5}us-west-2c", graph_attr=az_graph_attr):
        Node(label="", shape="plaintext", pin="true", pos="5.6,5" )
        Node(label="", shape="plaintext", pin="true", pos="6.6,7.2" )        
...
# This determines the outline used for the Public Subnet in AZ3
    with Cluster(f"{' '*8}Public Subnet\n\n10.0.2.0/24", graph_attr=public_graph_attr):
        Node(label="", shape="plaintext", pin="true", pos="6.4,6.8" )
        Node(label="", shape="plaintext", pin="true", pos="6.4,6.8" )  
        public_subnet_az3 = PublicSubnet(label="", fontsize="6", loc="t",fixedsize="true", width="0.3", height="0.3",
            pin="true", pos="5.2,7.4", )


# This determines the outline used for the Private Subnet in AZ3
    with Cluster(f"{' '*8}Private Subnet\n\n10.0.6.0/24", graph_attr=private_graph_attr):
        Node(label="", shape="plaintext", pin="true", pos="6.4,5.2" )
        Node(label="", shape="plaintext", pin="true", pos="6.4,5.2" )  
        private_subnet_az3 = PrivateSubnet(label="", fontsize="6", loc="t",fixedsize="true", width="0.3", height="0.3",
            pin="true", pos="5.2,5.8", )

# This determines the outline used for the ============Avalilabity Zone 4 ====================
    with Cluster(f"{' '*5}AZ4{' '*5}us-west-2d", graph_attr=az_graph_attr):
        Node(label="", shape="plaintext", pin="true", pos="9.4,5" )
        Node(label="", shape="plaintext", pin="true", pos="9.4,7.2" )        
        az4 = LocalZones(label="", fontsize="6", loc="t",fixedsize="true", width="0.3", height="0.3", 
            pin="true", pos="7.8,7.8")

# This determines the outline used for the Public Subnet in AZ4
    with Cluster(f"{' '*8}Public Subnet\n\n10.0.3.0/24", graph_attr=public_graph_attr):
        Node(label="", shape="plaintext", pin="true", pos="9.2,6.8" )
        Node(label="", shape="plaintext", pin="true", pos="8.6,6.8" )  
        public_subnet_az4 = PublicSubnet(label="", fontsize="6", loc="t",fixedsize="true", width="0.3", height="0.3",
            pin="true", pos="8,7.4", )


# This determines the outline used for the Private Subnet in AZ4
    with Cluster(f"{' '*8}Private Subnet\n\n10.0.7.0/24", graph_attr=private_graph_attr):
        Node(label="", shape="plaintext", pin="true", pos="9.2,5.2" )
        Node(label="", shape="plaintext", pin="true", pos="8.6,5.2" )  
        private_subnet_az4 = PrivateSubnet(label="", fontsize="6", loc="t",fixedsize="true", width="0.3", height="0.3",
            pin="true", pos="8,5.8", )
...
# Defining the Route Table Attributes for Public subnet
    routepub = RouteTable(label=f"{' '*6}PublicRouteTable\n\n\n\n", fontsize="8", loc="t",
            fixedsize="true", width="0.4", height="0.4", fontcolor="forestgreen",pin="true", pos="1.6,8.4")

# Connecting all public subnets to the route Table
    routepub - Edge(style="dotted",label="",color="forestgreen",) - [public_subnet_az1,public_subnet_az2,
                                                                     public_subnet_az3,public_subnet_az4]

# Defining the Route Table Attributes for Private subnet 
    routepriv = RouteTable(
            label=f"{' '*6}PrivateRouteTable\n\n\n\n", fontsize="8", loc="t",
            fixedsize="true", width="0.4", height="0.4", fontcolor="dodgerblue",pin="true", pos="7.4,3.9")

# Connecting all Private subnets to the route Table 
    routepriv - Edge(style="dotted",label="",color="dodgerblue",) - [private_subnet_az1,private_subnet_az2,
                                                                    private_subnet_az3,private_subnet_az4]