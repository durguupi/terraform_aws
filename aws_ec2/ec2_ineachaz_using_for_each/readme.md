...
# ✍Distribution of EC2 instance in Each AZ Using FOR_EACH

In this project we will distribute EC2 instances across the number of availability zones present in region using `for_each` meta-argument of terraform. The `for_each`, will generate as many copies of EC2 instances based on availability zones present. 
Understanding the logic used here using below example

```javascript

// local.az_names gives us list of AZ in region starting with index from 0 ,1 , 2. 
{ for idx, az_name in local.az_names : idx => az_name }
{
  "0" = "us-east-2a"
  "1" = "us-east-2b"
  "2" = "us-east-2c"
}
// Here idx refers to index,in this example we have for 3 AZ in region 
// so we have index starting from 0,1,2 and it corresponds to name of AZ. So for
// each AZ present we will create Public Subnet.
> { for idx, az_name in local.az_names : cidrsubnet(var.base_cidr, 8, idx) => az_name }
{
  "10.0.0.0/24" = "us-east-2a"
  "10.0.1.0/24" = "us-east-2b"
  "10.0.2.0/24" = "us-east-2c"
}
> { for idx, subnet in aws_subnet.public : idx => subnet.tags.Name }
{
  "0" = "Public-subnet-us-east-2a"
  "1" = "Public-subnet-us-east-2b"
  "2" = "Public-subnet-us-east-2c"
}
{for k, v in aws_instance.public_web :v.tags.Name => v.private_ip }
{
  "webserver1_us-east-2a" = "10.0.0.57"
  "webserver2_us-east-2b" = "10.0.1.57"
  "webserver3_us-east-2c" = "10.0.2.29"
}
```
...
![](scripts/distribution_of_ec2_instance_in_each_az_using_for_each.png)
...
cd aws_ec2/ec2_ineach_az_using_for_each
...
[<img src="screenshots/output1.png" width="800" height="500"/>](screenshots/output1.png) &nbsp;&nbsp;&nbsp;&nbsp; [<img src="screenshots/output2.png" width="500" height="500"/>](screenshots/output2.png)
